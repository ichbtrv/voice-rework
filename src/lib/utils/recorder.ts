...
export const recorder = async () => {
  ...
  const startRecordingEvent = document.addEventListener('recording:start', startRecording);
  const stopRecordingEvent = document.addEventListener('recording:stop', stopRecording);
